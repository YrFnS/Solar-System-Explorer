...
function createStarField() {
  ...
  const colorNode = instancedBufferAttribute<'vec3'>(colorAttribute, 'vec3')
  const sizeNode = instancedBufferAttribute<'float'>(sizeAttribute, 'float')
  const phaseNode = instancedBufferAttribute<'float'>(phaseAttribute, 'float')
  const twinkle = time
    .mul(0.82)
    .add(phaseNode)
    .sin()
...
function createSolarWindField() {
  ...
  const directionNode = instancedBufferAttribute<'vec3'>(directionAttribute, 'vec3')
  const tangentNode = instancedBufferAttribute<'vec3'>(tangentAttribute, 'vec3')
  const phaseNode = instancedBufferAttribute<'float'>(phaseAttribute, 'float')
  const speedNode = instancedBufferAttribute<'float'>(speedAttribute, 'float')
  const sizeNode = instancedBufferAttribute<'float'>(sizeAttribute, 'float')
  const temperatureNode = instancedBufferAttribute<'float'>(temperatureAttribute, 'float')
